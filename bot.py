...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin_user(update.effective_user):
        ...
        text = (
            "Добро пожаловать! Я бот для сбора обратной связи. "
            "Используйте /feedback, чтобы пройти опрос, "
            "/catfact для информации о породе и /catimage для фото кота."
        )
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Справка по командам."""
    is_admin = is_admin_user(update.effective_user)
    lines = [
        "Доступные команды:",
    ...
    if is_admin:
        lines.insert(3, "/report — сводный отчёт (только для руководителя)")
...
async def configure_public_commands(application: Application) -> None:
    """
    Настраивает список команд в главном меню для всех пользователей.
    /report не включается, так как это служебная команда только для руководителя.
    """
    await application.bot.set_my_commands(
        [
            BotCommand("start", "Приветствие и запуск бота"),
            BotCommand("feedback", "Пройти опрос"),
            BotCommand("catfact", "Случайная информация о породе кота"),
            BotCommand("catimage", "Случайное изображение кота"),
            BotCommand("dailycat", "Ежедневная информация о породе и фото"),
            BotCommand("cancel", "Прервать текущий опрос"),
...
def _telegram_loop_worker() -> None:
    ...
    telegram_loop.run_until_complete(configure_public_commands(telegram_application))
